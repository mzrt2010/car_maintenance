...
# استبدل هذه القيم ببيانات مشروعك الخاصة من Supabase (Settings -> API)
...
st.title("🚗 نظام إدارة وسجل صيانة السيارات (الاسكيمو الخاص)")

# --- 2. جلب البيانات من قاعدة البيانات باستخدام اسكيمو car_app ---
def get_cars():
    # استخدام .schema("car_app") للوصول للجدول الصحيح
    response = supabase.schema("car_app").table("cars").select("*").execute()
    return response.data

def get_maintenance_logs(car_id):
    # استخدام .schema("car_app") وتصفية السجلات حسب السيارة وترتيبها تنازلياً بالتاريخ
    response = (
        supabase.schema("car_app")
    ...
    with st.form("add_car_form", clear_on_submit=True):
        ...
        if submit_car:
            if car_name:
                ...
                # تحديد الاسكيمو car_app عند إدخال البيانات
                supabase.schema("car_app").table("cars").insert(data).execute()
        ...
        with st.form("add_maintenance_form", clear_on_submit=True):
            ...
            if submit_log:
                if details and cost > 0:
                    ...
                    # رفع الفواتير إلى Supabase Storage إن وجدت (تبقى كما هي لأن السحابة منفصلة عن الـ schemas)
                    if uploaded_files:
                    ...
                    # إدخال البيانات في جدول الصيانة التابع لـ car_app
                    log_data = {
                        "car_id": car_id,
                    ...
                    # تحديد الاسكيمو car_app هنا أيضاً عند الحفظ
                    supabase.schema("car_app").table("maintenance_log").insert(log_data).execute()
        ...
        with st.form("add_maintenance_form", clear_on_submit=True):
            ...
            if submit_log:
                if details and cost > 0:
                    ...
                    # رفع الفواتير إلى Supabase Storage إن وجدت
                    if uploaded_files:
                        for file in uploaded_files:
                            ...
                            try:
                                # رفع الملف إلى الـ Bucket المسمى 'invoices'
                                supabase.storage.from_("invoices").upload(unique_filename, file.read())
                    ...
                    # إدخال البيانات في جدول الصيانة
                    log_data = {
                        "car_id": car_id,
                        "maintenance_date": str(m_date),
                        "details": details,
...
with tab1:
    ...
    if not car_options:
        ...
        if not logs:
            ...
            # عرض السجلات بشكل بطاقات (Cards) أو جدول منظم
            for log in logs:
                with st.container():
                    col1, col2, col3 = st.columns([2, 4, 2])
                    with col1:
                    ...
                    # عرض الفواتير المرفقة إن وجدت
                    if log.get('invoice_urls'):
                        st.write("**الفواتير المرفقة:**")
                        cols_inv = st.columns(len(log['invoice_urls']))
                        for idx, url in enumerate(log['invoice_urls']):
                            with cols_inv[idx]:
                                # إتاحة رابط التحميل/المشاهدة
                                st.markdown(f"[📄 فاتورة {idx+1}]({url})", unsafe_allow_html=True)
                                # إذا كانت الصورة مدعومة يمكن عرضها مصغرة
                                if any(ext in url.lower() for ext in ['jpg', 'jpeg', 'png']):
                                    st.image(url, width=100)
                    st.divider()
